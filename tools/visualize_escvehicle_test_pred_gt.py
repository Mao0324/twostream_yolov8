#!/usr/bin/env python3
"""
随机抽样 escVehicle 测试集图像，绘制 GT 与预测框（OBB）并保存。

示例:
python tools/visualize_escvehicle_test_pred_gt.py \
  --weights /path/to/best.pt \
  --data data/escvehicle.yaml \
  --num-samples 10 \
  --device 0
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Visualize escVehicle test predictions and GT.")
    parser.add_argument("--weights", type=str, required=True, help="Path to model weights, e.g. best.pt")
    parser.add_argument(
        "--data",
        type=str,
        default=str(repo_root / "data" / "escvehicle.yaml"),
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default=str(repo_root / "vis" / "escvehicle_test_vis"),
...
def to_label_path_from_image(image_path: Path) -> Path:
    p = str(image_path)
    p = p.replace("/images/", "/labels/")
    p = p.replace("\\images\\", "\\labels\\")
    return Path(str(Path(p).with_suffix(".txt")))


def load_test_pairs(data_yaml: Path) -> tuple[list[tuple[Path, Path, Path]], dict[int, str]]:
    with data_yaml.open("r", encoding="utf-8") as f:
        cfg = yaml.safe_load(f)

    root = Path(cfg.get("path", data_yaml.parent)).resolve()
    test_src = resolve_path(root, cfg["test"])
    test_ir_src = resolve_path(root, cfg["test_ir"])
    ...
    if test_src.is_file() and test_ir_src.is_file():
        rgb_lines = [Path(x.strip()) for x in test_src.read_text(encoding="utf-8").splitlines() if x.strip()]
        ir_lines = [Path(x.strip()) for x in test_ir_src.read_text(encoding="utf-8").splitlines() if x.strip()]
        n = min(len(rgb_lines), len(ir_lines))
        for i in range(n):
            rgb_p = rgb_lines[i]
            ir_p = ir_lines[i]
            lb_p = to_label_path_from_image(rgb_p)
            pairs.append((rgb_p, ir_p, lb_p))
        return pairs, names

    if not test_src.exists():
        raise FileNotFoundError(f"Test source not found: {test_src}")
    if not test_ir_src.exists():
        raise FileNotFoundError(f"Test IR source not found: {test_ir_src}")

    rgb_images = sorted([p for p in test_src.iterdir() if p.is_file() and p.suffix.lower() in exts])
    for rgb_p in rgb_images:
        ir_p = find_ir_pair(test_ir_src, rgb_p)
        if ir_p is None:
            continue
        lb_p = to_label_path_from_image(rgb_p)
        pairs.append((rgb_p, ir_p, lb_p))
...
def find_ir_pair(ir_dir: Path, rgb_path: Path) -> Path | None:
    direct = ir_dir / rgb_path.name
    if direct.exists():
        return direct
    candidates = sorted(ir_dir.glob(f"{rgb_path.stem}.*"))
...
def main() -> int:
    ...
    all_pairs, names = load_test_pairs(data_yaml)
    if not all_pairs:
        raise RuntimeError("No valid test RGB-IR pairs found from dataset yaml.")

    k = min(args.num_samples, len(all_pairs))
    chosen_pairs = random.sample(all_pairs, k=k)

    save_dir = Path(args.save_dir).resolve()
    save_dir.mkdir(parents=True, exist_ok=True)
    chosen_txt = save_dir / "selected_images.txt"
    chosen_txt.write_text("\n".join(str(rgb) for rgb, _, _ in chosen_pairs), encoding="utf-8")

    model = YOLO(args.weights)

    for img_path, ir_path, label_path in chosen_pairs:
        ...
        if ir_img is None:
            print(f"[WARN] skip unreadable IR image: {ir_path}")
            continue
